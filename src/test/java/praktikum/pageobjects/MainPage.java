...
public class MainPage {
    ...
    public boolean isBunsTabActive() {
        WebElement bunsTab = webDriver.findElement(bunsButton);
        return bunsTab.getAttribute("class").contains("current");
    }

    // Проверка активности таба "Соусы"
    public boolean isSaucesTabActive() {
        WebElement saucesTab = webDriver.findElement(saucesButton);
        return saucesTab.getAttribute("class").contains("current");
    }

    // Проверка активности таба "Начинки"
    public boolean isFillingsTabActive() {
        WebElement fillingsTab = webDriver.findElement(fillingsButton);
        return fillingsTab.getAttribute("class").contains("current");
    }

    // Проверка видимости таба "Булки"
    public boolean isBunsTabVisible() {
        return webDriver.findElement(bunsButton).isDisplayed();
    }

    // Проверка видимости таба "Соусы"
    public boolean isSaucesTabVisible() {
        return webDriver.findElement(saucesButton).isDisplayed();
    }

    // Проверка видимости таба "Начинки"
    public boolean isFillingsTabVisible() {
        return webDriver.findElement(fillingsButton).isDisplayed();
    }

    // Проверка видимости и активности таба "Булки"
    public boolean isBunsTabActiveAndVisible() {
        return isBunsTabVisible() && isBunsTabActive();
    }

    // Проверка видимости и активности таба "Соусы"
    public boolean isSaucesTabActiveAndVisible() {
        return isSaucesTabVisible() && isSaucesTabActive();
    }

    // Проверка видимости и активности таба "Начинки"
    public boolean isFillingsTabActiveAndVisible() {
        return isFillingsTabVisible() && isFillingsTabActive();
    }
